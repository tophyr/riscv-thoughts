...
ssh ${REMOTE} "cd Projects/riscv-thoughts && \
  ${PYTHON} scripts/mux_batches.py --gen instr --gen-count 80 --batch-size ${BATCH_SIZE} --n-batches ${NUM_BATCHES} \
  | lz4 | nc -l -q 0 6464" &
SSH_PID=$!
...
# Second mux: mix in branch-heavy batches at weight 0.5 (~10% of
# branch instructions come from focused context).
${PYTHON} scripts/mux_batches.py --gen instr --gen-count 16 --batch-size ${BATCH_SIZE} --n-batches ${NUM_BATCHES} \
    <(nc odin 6464 -d | unlz4) | \
  ${PYTHON} scripts/batch_slice.py --count ${NUM_BATCHES} | \
  ${PYTHON} scripts/train_compressor.py --mode instr --n-steps ${NUM_BATCHES}
