...
Codifies the manual grading run as five subprocess phases:
  ...
  3. cat corpus | train_encoder — train an encoder from random init
  4. cat corpus | train_decoder — train a decoder on the frozen encoder
...
class TestEndToEndGrading:
    ...
    def test_phase3_train_encoder(self, workspace):
        """`cat corpus | train_encoder` → encoder run dir with checkpoints."""
        corpus = workspace / 'corpus.rvt'
    ...
    def test_phase4_train_decoder(self, workspace):
        """`cat corpus | train_decoder --t1-model encoder.pt` → decoder.pt."""
        corpus = workspace / 'corpus.rvt'
        encoder = workspace / 'encoder' / 'encoder.pt'
        dec_dir = workspace / 'decoder'
        assert encoder.exists(), 'phase 3 must run first'
        cmd = (
            f'cat {corpus} | '
            f'{PYTHON} scripts/train_decoder.py --t1-model {encoder} '
            f'--d-model {D_MODEL} --n-heads {N_HEADS} '
            f'--n-layers {N_LAYERS} '
            f'--n-steps {N_STEPS} --log-every {LOG_EVERY} '
            f'--out-dir {dec_dir}'
        )
        ...
        assert (dec_dir / 'decoder.pt').exists()
        assert (dec_dir / 'hparams.json').exists()
        assert (dec_dir / 'losses.json').exists()
        assert b'Saved to' in out
    ...
    def test_phase5_acceptance_suite(self, workspace):
        ...
        for this toy run."""
        encoder = workspace / 'encoder' / 'encoder.pt'
        decoder = workspace / 'decoder' / 'decoder.pt'
