...
**Pipeline tools:** `batch_slice` (inspect/subset), `mux_batches`
(parallel generation + weighted/interleaved mixing). RVT batches are
self-describing (no stream header), so plain `cat` concatenates or
repeats streams. All format-agnostic.
